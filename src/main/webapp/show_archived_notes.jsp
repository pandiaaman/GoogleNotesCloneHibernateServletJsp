...
<body>
	...
	<%
		...
		if(session.getAttribute("user") == null){
			session.setAttribute("errormsg", "please login to see your archived notes");
			response.sendRedirect("index.jsp");
		}
		
		//session timeout in seconds after inactivity on same session
		session.setMaxInactiveInterval(2*60); //5 mins of inactivity on this page will lead to session timeout user has to login again
	...
	<div class="container">
		...
		<%
			...
			Query q = sess.createQuery("from Note where user_id =:x");
			q.setParameter("x", session.getAttribute("userid"));
			
			List<Note> notes = q.list();
			
			for(Note note : notes){
				if(note.isNotePinned() == true && note.isArchived() == true){
				%>
				   ...
				   <a href="edit_note.jsp?noteid=<%=note.getId()%>">
					<span style="position:absolute; width:100%;height:100%;top:0;left: 0;z-index: 1;">
				  	<div class="container text-right">
				  		<a class="pr-3" href="ArchiveNoteServlet?noteid=<%=note.getId()%>&value=0"><img src="unarchive.png" title="unarchive note" alt="unarchive icon" width="20" height="20" style="position : relative; z-index: 2;"/></a>
