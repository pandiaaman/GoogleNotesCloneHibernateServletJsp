<nav class="navbar navbar-expand-lg navbar-light bg-light sticky-top">
		  <a class="navbar-brand" href="index.jsp">Note Taker</a>
		  <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
		    <span class="navbar-toggler-icon"></span>
		  </button>
		
		  <div class="collapse navbar-collapse" id="navbarSupportedContent">
		    <ul class="navbar-nav mr-auto">
		      <li class="nav-item active">
		        <a class="nav-link" href="index.jsp">Home <span class="sr-only">(current)</span></a>
		      </li>
		     
		      <li class="nav-item">
		      <%
				if(session.getAttribute("uname") == null){
		
				%>
		        <a class="nav-link" href="userregister.jsp">Register</a>
		        
		        <%
				}
		        else{
		
				%>
				
					<a class="nav-link" href="show_all_notes.jsp">Show all notes</a>
				
				</li>
				 
				<li class="nav-item">
				 
					<a class="nav-link" href="show_archived_notes.jsp">Archived notes</a>				
				<%
		        }
		
				%>
		        
		      </li>
		      
		      
		       <li class="nav-item">
			    <%
				if(session.getAttribute("uname") == null){
		
				%>
		        <a class="nav-link" href="userlogin.jsp">Login</a>
		        
		        <%
				}
		        else{
		
				%>
				
					<a class="nav-link" href="add_notes.jsp">add Note</a>
				</li>
				<li class="nav-item">
					<a class="nav-link" href="lougout.jsp">Logout</a>
				<%
		        }
		
				%>
		        
		        
		      </li>
		      
		      <li class="nav-item dropdown">
		        <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
		          About Us
		        </a>
		        <div class="dropdown-menu" aria-labelledby="navbarDropdown">
		          <a class="dropdown-item" href="#">Our Team</a>
		          <a class="dropdown-item" href="#">Contact us</a>
		          <div class="dropdown-divider"></div>
		          <a class="dropdown-item" href="#">Query or Grievences</a>
		        </div>
		      </li>
		      
		    </ul>
		    <form class="form-inline my-2 my-lg-0" action="./SearchNoteServlet" method="post">
		      <input class="form-control mr-sm-2" name="searchStr" type="search" placeholder="Search" aria-label="Search">
		      <button class="btn btn-outline-success my-2 my-sm-0" type="submit">Search</button>
		    </form>
		  </div>
		</nav>